...
<div class="max-w-4xl mx-auto">
    ...
    <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-200 mb-2">{{ __('Gate Dashboard') }}</h1>
        <p class="text-gray-400">{{ __('Scan a QR code or enter a serial number below to verify tickets.') }}</p>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-4 text-center">
            ...
            <p class="text-2xl font-bold text-yellow-300">{{ $stats['used'] }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <button onclick="openScanner()" class="bg-red-500 hover:bg-red-600 text-white py-6 px-6 rounded-xl font-bold text-xl flex flex-col items-center gap-3 transition transform hover:scale-105 shadow-lg shadow-red-500/30">
            <span class="text-3xl">📷</span>
            {{ __('Scan QR Code') }}
        </button>
        <a href="{{ route('gate.verify') }}" class="bg-gray-800 hover:bg-gray-700 text-white py-6 px-6 rounded-xl font-bold text-xl flex flex-col items-center gap-3 transition transform hover:scale-105 border border-gray-700">
            <span class="text-3xl">🔢</span>
