    ...
    <div class="container mx-auto px-4 max-w-3xl">
        <div class="mb-8">
            <div class="flex justify-between mb-2">
                <span class="text-sm font-medium text-gray-300">{{ __('Step 1') }} — {{ __('Ticket Details') }}</span>
                <span class="text-sm font-medium text-gray-300">{{ __('Step 2') }} — {{ __('Payment') }}</span>
                <span class="text-sm font-medium text-gray-300">{{ __('Step 3') }} — {{ __('Upload Proof') }}</span>
                <span class="text-sm font-medium text-gray-300">{{ __('Step 4') }} — {{ __('Verification') }}</span>
                <span class="text-sm font-medium text-gray-300">{{ __('Step 5') }} — {{ __('Ticket') }}</span>
            </div>
        ...
        <form method="POST" action="{{ route('booking.store') }}" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="bg-gray-950 rounded-xl shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-300 mb-4">{{ __('Customer Information') }}</h2>
            ...
            <div class="bg-gray-950 rounded-xl shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-300 mb-4">{{ __('Select Ticket Type') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="border-2 rounded-lg p-4 text-center cursor-pointer transition {{ $selectedTypeId == $type->id ? 'border-red-600 bg-red-50' : 'border-gray-800 hover:border-red-400' }}">
                        ...
                        <label for="type_{{ $type->id }}" class="cursor-pointer">
                            ...
                            <p class="text-sm text-gray-300">{{ Str::limit($type->description_en, 80) }}</p>
                            <p class="text-xs text-gray-300 mt-2">{{ __('Capacity') }}: {{ $type->capacity }}</p>
                        </label>
                    </div>
                    @endforeach
                </div>
            ...
            <!-- Quantity -->
            <div class="bg-gray-950 rounded-xl shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-300 mb-4">{{ __('Select Quantity') }}</h2>
                <div class="flex items-center justify-center space-x-6">
                    <button type="button" id="minus" class="w-12 h-12 bg-gray-200 rounded-full text-2xl font-bold text-gray-300 hover:bg-gray-300 transition">-</button>
                ...
                <div class="text-center mt-4">
                    <p class="text-sm text-gray-300">{{ __('Ticket Price') }}:</p>
                ...
                <div class="text-center mt-4">
                    <p class="text-sm text-gray-300">{{ __('Total') }}:</p>
            ...
            <!-- Payment Method -->
            <div class="bg-gray-950 rounded-xl shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-300 mb-4">{{ __('Payment Method') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="border-2 rounded-lg p-4 flex items-center space-x-4 cursor-pointer transition {{ old('payment_method') == 'vodafone_cash' ? 'border-red-600 bg-red-50' : 'border-gray-800 hover:border-red-400' }}">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-300 font-mono">{{ \App\Models\Setting::cached('payment', 'vodafone_cash_number') }}</p>
                        </div>
                    </label>
                    <label class="border-2 rounded-lg p-4 flex items-center space-x-4 cursor-pointer transition {{ old('payment_method') == 'instapay' ? 'border-red-600 bg-red-50' : 'border-gray-800 hover:border-red-400' }}">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-300 font-mono">{{ \App\Models\Setting::cached('payment', 'instapay_account') }}</p>
                        </div>
                    </label>
                </div>
            ...
            <!-- Transfer Phone -->
            <div class="bg-gray-950 rounded-xl shadow-lg p-6">
                <h2 class="text-xl font-bold text-gray-300 mb-4">{{ __('Payment Details') }}</h2>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium mb-1">{{ __('Transfer Phone') }}</label>
                        <p class="text-xs text-gray-300 mb-1">{{ __('Phone number/account you transferred the money from') }}</p>
            ...
            <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white font-bold text-xl py-4 px-6 rounded-xl shadow-lg shadow-red-500/30 hover:shadow-red-500/40 transition">{{ __('Submit Request') }}</button>
        </form>
    </div>
</div>
